...
type Options struct {
	SendNo            int               `json:"sendno"`              //推送序号
	TimeToLive        int               `json:"time_to_live"`        //离线消息保留时长(秒)
	OverrideMsgId     int               `json:"override_msg_id"`     //要覆盖的消息 ID
	ApnsProduction    bool              `json:"apns_production"`     //APNs 是否生产环境
	ApnsCollapseId    string            `json:"apns_collapse_id"`    //更新 iOS 通知的标识符
	BigPushDuration   int               `json:"big_push_duration"`   //定速推送时长(分钟)
	ThirdPartyChannel ThirdPartyChannel `json:"third_party_channel"` //推送请求下发通道
}
